...
public class WeatherServlet extends HttpServlet {
  ...
  // The secret name is read from an environment variable to support
  ...
  // Fallback: environment variable key name for the Weather API key
  // (used when Secrets Manager is not available, e.g., local development)
   ...
   * Replaces hardcoded/embedded credentials with centralized secret management
   * (cr-java-0113 - Lack of Externalized Secrets).
   */
  private String getWeatherApiKey() {
    // First attempt: retrieve from AWS Secrets Manager for production use
    ...
    // Fallback: read from environment variable (12-factor app principle)
    String apiKey = System.getenv(WEATHER_API_KEY_ENV);
    if (apiKey != null && !apiKey.trim().isEmpty()) {
      logger.info("Weather API key retrieved from environment variable.");
      return apiKey.trim();
    }

    logger.info("Weather API key not found in Secrets Manager or environment variable.");
    return null;
  }
