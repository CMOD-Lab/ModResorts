...
public class AvailabilityCheckerServlet extends HttpServlet {
  ...
  protected int exportRevervations(String selectedDateStr) {
    // Read reservations.json from classpath resource via IOUtils
    byte[] jsonBytes = IOUtils.getResourceBytes("reservations.json");
    if (jsonBytes == null) {
      return -1;
    }

    // Build ZIP in memory using try-with-resources (no local file system dependency)
    byte[] zipBytes;
