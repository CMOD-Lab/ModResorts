...
public class AvailabilityCheckerServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    if (!parsedDate || reservationCheckerData.getReservationList() == null) {
      ...
      for (Reservation reservation : reservations) {
        try {
          // Use java.time API with UTC standardization (replaces java.util.Date)
  ...
  protected int exportRevervations(String selectedDateStr) {
    // Read reservations content from classpath resource via IOUtils
    byte[] reservationsContent = IOUtils.getResourceAsBytes("reservations.json");
    if (reservationsContent == null) {
      return -1;
    }

    // Use try-with-resources to ensure all streams are automatically closed
    // (fixes resource leak blocker cr-java-0098)
    try (ByteArrayOutputStream baos = new ByteArrayOutputStream();
         ZipOutputStream zipOut = new ZipOutputStream(baos)) {

      // Write reservations.json content into the ZIP in-memory
      ZipEntry zipEntry = new ZipEntry("reservations.json");
      zipOut.putNextEntry(zipEntry);

      byte[] bytes = new byte[1024];
      int offset = 0;
      while (offset < reservationsContent.length) {
        int length = Math.min(bytes.length, reservationsContent.length - offset);
        zipOut.write(reservationsContent, offset, length);
        offset += length;
      }
      zipOut.closeEntry();
      zipOut.finish();
